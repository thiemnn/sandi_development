...
const insert = async (body) => {
    try {
        ...
        await Promise.all(body.materials.map(async (material) => {
            var sqlDetail = `INSERT INTO m_stock_transaction_detail (
                transaction_id, 
                material_code, 
                quantity,
                price,
...
const update = async (id, body) => {
    try {
        ...
        await con.query('Delete from m_stock_transaction_detail where transaction_id = ' + id)
...
const get = async (_id) => {
    try {
        ...
        const transaction_details = await con.query(`SELECT *, B.name as material_name, C.name as position_name FROM sandi_stock_db.m_stock_transaction_detail as A
